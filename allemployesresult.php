...
print("
<td ><a href = 'updateempinfo.php?emp_id=".$row['emp_id']."'>[Edit]</a><a href = 'deleteemployee.php?emp_id=".$row['emp_id']."'><p onclick='isdelete();'>[Delete]</p></a></td>
		");
...
$db->close();
 ...
 echo'<p align="center">';
 if ($current_page != 1) {
 echo '<a href="allemployesresult.php?s=' . ($start - $display) .'&p=' . $pages . '">Previous</a> ';
 }
 for ($i = 1; $i <= $pages; $i++) {
 if ($i != $current_page) {
 echo '<a href="allemployesresult.php?s=' . (($display *($i - 1))) . '&p=' . $pages .'">' . $i . '</a> ';
 } else {
 echo $i . ' ';
 }
 } 
 if ($current_page != $pages) {
 echo '<a href="allemployesresult.php?s=' . ($start + $display) .'&p=' . $pages . '">Next</a>';
 }
 echo '</p>'; 
 echo '</p>'; 
 } 
